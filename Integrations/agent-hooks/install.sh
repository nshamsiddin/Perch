#!/usr/bin/env bash
#
# Islet - install AI agent tracking hooks for Claude Code and Cursor.
#
# Idempotent and non-destructive: safe to run repeatedly. It copies the status-writer scripts into
# place and adds Islet's entries to the Claude Code / Cursor hook configs only if they're missing,
...
echo "Installing Islet agent-tracking hooks..."

# 1. Copy the hook scripts into place (executable) and ensure the state dirs exist.
