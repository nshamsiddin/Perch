...
struct IslandRootView: View {
    ...
    private var layout: IslandLayout { IslandLayout(geometry: state.geometry) }
    ...
    /// Effective visual presentation derived from mode + transient activity + now-playing.
    /// Precedence while collapsed: volume HUD > activity peek > now-playing compact > bare pill.
    private enum Presentation { case collapsed, nowPlayingCompact, peek, volumeHUD, expanded }
    ...
    private var presentation: Presentation {
        ...
        if state.nowPlaying.hasContent { return .nowPlayingCompact }
        return .collapsed
    }
    ...
    private var islandSize: CGSize {
        ...
        case .expanded:
            return CGSize(width: layout.expandedWidth - 24, height: layout.expandedHeight - 14)
        }
    }

    private var cornerRadii: RectangleCornerRadii {
        switch presentation {
        case .collapsed, .nowPlayingCompact:
    ...
    private var island: some View {
        ...
        .shadow(color: .black.opacity(presentation == .expanded ? theme.shadowOpacity : 0), radius: 12, y: 6)
    }
